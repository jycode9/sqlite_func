...
public:
	...
	//更新：全部修改
	int updateData(const std::string& tablename, void* columns, void* values);
	//更新：where索引
	...
	template <typename T>
	int selectData123(const std::string& tablename, std::vector<T>& calldata);



	//查找：全部查找
	int selectData(const std::string tablename, std::vector<std::map<std::string, std::string>>& calldatas);
	////查找：where索引全部查找
...
template <typename T>
int sqlfunc::selectData123(const std::string& tablename, std::vector<T>& calldata) {
	//SELECT * FROM COMPANY;
	std::string sql_tablename = tablename;
	std::string sql_word = "SELECT * FROM " + sql_tablename + ";";
	...
	for (int i = 0; i < SelectData::select_data_num; i++) {
		std::map<std::string, std::string> t_calldata;
...
int sqlfunc::selectData(const std::string tablename, std::vector<std::map<std::string, std::string>>& calldatas) {
